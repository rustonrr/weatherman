...
class App extends Component {
	renderChildren() {
		...
		if(loading) {
			return ( <img alt="loading indicator" src={ hourglass } />)
		}
		if(search) {
			return <EnterLocation />
		}
		return ( <CurrentWeather reset={ reset } weather={ weather } />)
	}
	...
	render() {
		...
		return (
			<div className="app">
				<h1 className="app__title">RUSTON THE G.O.A.T. WEATHERMAN</h1>
				{ this.renderChildren() }
			</div>
		);
	}
